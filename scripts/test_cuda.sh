...
# Step 5 fused attention + Step 6 full GPU forward pass + Step 7 greedy decode loop. Later
# kernel tests join this -R filter. test_forward_gpu / test_generate_gpu SKIP green without
# the local weight blob.
"$CTEST" --test-dir build-cuda -R 'test_device|test_matmul|test_rmsnorm|test_kernels|test_attention_gpu|test_forward_gpu|test_generate_gpu' --output-on-failure
